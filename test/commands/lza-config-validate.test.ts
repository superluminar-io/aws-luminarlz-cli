...
describe('LzaConfigValidate Command', () => {
  ...
  test('should validate the synthesized LZA config', async () => {
    // Create an instance of the LzaConfigValidate command
    const lzaConfigValidate = new LzaConfigValidate();

    // Execute the command
    await lzaConfigValidate.execute();
    ...
    expect(executeCommand).toHaveBeenCalledWith(
      `yarn validate-config ${path.join(tempDir, 'aws-accelerator-config.out')}`,
      {
        cwd: expect.stringContaining('/mock/checkout/path'),
      },
    );
