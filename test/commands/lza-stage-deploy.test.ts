...
import { LzaStageDeploy } from '../../src/commands/lza-stage-deploy';
...
import { ensureCheckoutExists, getCheckoutPath } from '../../src/core/accelerator/repository/checkout';
...
describe('LzaStageDeploy Command', () => {
  ...
  test('should deploy a stage with default stage value', async () => {
    // Create an instance of the LzaStageDeploy command
    const command = new LzaStageDeploy();
    ...
    expect(executeCommand).toHaveBeenCalledWith(
      expect.stringMatching(/yarn run ts-node --transpile-only cdk\.ts synth --stage .*/),
      expect.any(Object),
    );

    // For deployStage
    expect(executeCommand).toHaveBeenCalledWith(
      expect.stringMatching(/yarn run ts-node --transpile-only cdk\.ts deploy --require-approval never --stage .*/),
      expect.any(Object),
    );

    // Verify console.log was called with the right message
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/Deployed AWS Accelerator .* stage\. ✅/));
  });

  test('should deploy a stage with specified stage value', async () => {
    // Create an instance of the LzaStageDeploy command
    const command = new LzaStageDeploy();
    ...
    expect(ensureCheckoutExists).toHaveBeenCalled();
    expect(executeCommand).toHaveBeenCalledWith(
      expect.stringMatching(/yarn run ts-node --transpile-only cdk\.ts synth --stage operations/),
      expect.any(Object),
    );

    // For deployStage
    expect(executeCommand).toHaveBeenCalledWith(
      expect.stringMatching(/yarn run ts-node --transpile-only cdk\.ts deploy --require-approval never --stage operations/),
      expect.any(Object),
    );

    // Verify console.log was called with the right message
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/Deployed AWS Accelerator operations stage\. ✅/));
  });
});
